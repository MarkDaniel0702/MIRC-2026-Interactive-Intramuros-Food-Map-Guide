...
export function Masthead({ mode, plm }: { mode: ModeKey; plm: boolean }) {
  return (
    <header className="masthead">
      <p className="masthead__eyebrow">Manila &middot; The Walled City</p>
      <h1 className="masthead__title">{plm ? 'PLM Map' : 'Intramuros Map'}</h1>
      <p className="masthead__sub">{plm ? 'The MIRC 2026 venue and its campus' : MODES[mode].subtitle}</p>
